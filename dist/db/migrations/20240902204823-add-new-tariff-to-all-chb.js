...
function up(queryInterface) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const [customerHasBanks] = yield queryInterface.sequelize.query(`SELECT id_customer_has_bank FROM ${CUSTOMER_HAS_BANK_TABLE} WHERE id_customer_has_bank IS NOT NULL AND id_customer_has_bank != 1`);
            if (!customerHasBanks.length)
                return;
            yield Promise.all(customerHasBanks.map((customerHasBank) => __awaiter(this, void 0, void 0, function* () {
                var _a;
                const newTariffs = tariff.map((tariff) => (Object.assign(Object.assign({}, tariff), { customer_has_bank_id_customer_has_bank: customerHasBank.id_customer_has_bank })));
                yield queryInterface.bulkInsert(TARIFF_TABLE, newTariffs);
                const tariffId = yield queryInterface.sequelize.query(`SELECT id_tariff FROM ${TARIFF_TABLE} WHERE customer_has_bank_id_customer_has_bank = ${customerHasBank.id_customer_has_bank}`, { type: sequelize_1.QueryTypes.SELECT });
                const newTariffInterMatch = [];
                if (!Array.isArray(tariffId))
                    return;
                for (let i = 0; i < 29; i++) {
                    if (((_a = tariffId[i]) === null || _a === void 0 ? void 0 : _a.id_tariff) !== undefined) {
                        const findIntervals = tariffIntervalMatch.filter((item) => item.tariff_id === i + 1);
                        if (!findIntervals.length)
                            return;
                        findIntervals.forEach((item) => {
                            var _a;
                            newTariffInterMatch.push(Object.assign(Object.assign({}, item), { tariff_id: (_a = tariffId[i]) === null || _a === void 0 ? void 0 : _a.id_tariff }));
                        });
                    }
                }
                yield queryInterface.bulkInsert(TARIFF_INTERVAL_MATCH_TABLE, newTariffInterMatch);
            })));
        }
        catch (error) {
            console.log(error);
